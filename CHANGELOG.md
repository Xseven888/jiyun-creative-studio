...
+ [新增] 画布 Agent 新增浏览器内置模式，可直接使用已配置的文本模型操作画布，并保留本地 Codex 模式切换。
+ [调整] 顶部导航品牌更新为“即云创作台”，浏览器标签页使用新的彩色标识并移除右上角 GitHub、文档和版本入口。
+ [调整] 内置 Agent 默认使用 `gpt-5.6-sol`，默认渠道改为 `https://api.dreayun.com`，画布默认生图张数改为 1，默认视频模型改为 `veo_3_1-components`。
+ [调整] 首次访问默认使用英文界面；配置渠道固定使用 DreaYun 接口并移除新增渠道和接口地址输入，API Key 支持打开令牌页及从剪贴板导入。
