...
export async function requestTextToolTurn(
    config: AiConfig,
    input: {
        systemPrompt: string;
    ...
    if (resolveModelScript(config, model)) throw new Error("内置 Agent 暂不支持文本模型的自定义调用脚本，请使用标准 OpenAI Responses 或 Gemini 渠道");
    const systemPrompt = [requestConfig.systemPrompt.trim(), input.systemPrompt.trim()].filter(Boolean).join("\n\n");
    ...
    try {
        if (requestConfig.apiFormat === "gemini") {
            return await requestGeminiStreamingResponse(
                ...
                toGeminiBody(
                    ...
                    toGeminiToolOptions(input.tools, input.toolChoice || "auto"),
                ),
                input.onDelta,
                options,
            );
        ...
        if (input.signal?.aborted) throw new DOMException("请求已停止", "AbortError");
        throw new Error(readAxiosError(error, apiText("requestFailed")));
    }
}
